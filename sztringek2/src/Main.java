public class Main
{
    ...
    private static String notBad(String s){
        if(s.indexOf("not")<s.indexOf("bad") && s.indexOf("not")!=-1 && s.indexOf("bad")!=-1){
            return s.replace(s.substring(s.indexOf("not"), s.indexOf("bad")+3), "good");
        }else{
            return s;
        }
    }
    ...
    private static String frontBack(String a, String b){
        ...
        return a.substring(0,a_length/2)+b.substring(0,b_length/2)+a.substring(a_length/2)+b.substring(b_length/2);
    }
